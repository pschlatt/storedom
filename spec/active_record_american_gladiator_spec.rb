...
describe "ActiveRecord American Gladiator" do
  ...
  context "The Eliminator" do
    it "returns all orders placed 2 weeks ago" do
      ...
      # end
      orders = Order.where("DATE(created_at) BETWEEN ? AND ?", two_weeks_ago, last_week)
      # Changeable End
  ...
  context "Atlasphere" do
    ...
    it "returns most popular items" do
      ...
      #   Item.find(id)
      # end
      most_popular_items = Item.select("items.*, count(order_items.item_id) AS items_counted").joins(:order_items).
                          group("items.id").
                          order("items_counted DESC LIMIT 2")
